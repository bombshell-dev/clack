...
export const spinner = ({ indicator = 'dots' }: SpinnerOptions = {}) => {
	...
	let _prevFormattedMessage: string | undefined = undefined;
	...
	const clearPrevMessage = (): void => {
		if (_prevFormattedMessage === undefined) return;
		if (isCI) process.stdout.write('\n');
		const linesCounter = _prevFormattedMessage.split(/\n/g).length;
	...
	const start = (msg = ''): void => {
		...
		loop = setInterval(() => {
			...
			if (isCI) {
				_prevFormattedMessage = formatMessage(frame, `${_message}...`);
				process.stdout.write(_prevFormattedMessage);
			} else if (indicator === 'timer') {
				_prevFormattedMessage = formatMessage(frame, `${_message} ${formatTimer(_origin)}`);
				process.stdout.write(_prevFormattedMessage);
			} else {
				const loadingDots = '.'.repeat(Math.floor(indicatorTimer)).slice(0, 3);
				_prevFormattedMessage = formatMessage(frame, `${_message}${loadingDots}`);
				process.stdout.write(_prevFormattedMessage);
			}
