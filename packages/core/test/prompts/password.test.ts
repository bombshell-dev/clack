...
describe('PasswordPrompt', () => {
	...
	describe('userInputWithCursor', () => {
		...
		test('renders marker at end', () => {
			...
			expect(instance.userInputWithCursor).to.equal(`•${styleText('inverse', styleText('hidden', '_'))}`);
		});
		...
		test('renders custom mask', () => {
			...
			instance.prompt();
			input.emit('keypress', 'x', { name: 'x' });
			expect(instance.userInputWithCursor).to.equal(`X${styleText('inverse', styleText('hidden', '_'))}`);
		});
	});
});
